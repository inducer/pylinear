...
\begin{funcdesc}{zeros}{shape, typecode=Float, flavor=None}
...
\begin{funcdesc}{ones}{shape, typecode=Float, flavor=None}
...
\begin{funcdesc}{eye}{n, m=None, offset=0, typecode=Float, flavor=None}
...
\begin{verbatim}
>>> eye(4, 3, offset=1, typecode=Complex)
\end{verbatim}
\end{funcdesc}

\begin{funcdesc}{tri}{n, m=None, offset=0, typecode=Float, flavor=None}
  \function{tri} creates a \class{Matrix} of shape \code{(n,m)} and
  the given \var{typecode} and \var{flavor} which has ones on the 
  \code{offset}th super-diagonal and below, and zeros elsewhere.

  \code{offset} may also be negative, thus pointing to a sub-diagonal.

\begin{verbatim}
>>> tri(4, 3, offset=1, typecode=Complex)
\end{verbatim}
\end{funcdesc}
...
\section{Extracting parts of \class{Array}s}
...
\class{Matrix} objects are indexed by 2-tuples, whereas vectors are indexed by
...
supported, so that you can write \code{a[3:17]}, \code{a[1:3,5:9]}, or even
\code{a[::-1]} to obtain the reverse of a \class{Vector}. Unlike Python lists,
\class{Array}s may not be resized using slice assignments. Like in the rest of
...
\section{More \class{Matrix} operators}
