...
\begin{methoddesc}[Matrix]{set_element}{i, j, value}
  Only on \class{DenseMatrix}, \class{SparseBuildMatrix}. Basically,
  this element is equivalent to \code{self[i,j] = value},
  however it runs slightly faster on \class{SparseBuildMatrix}
  instances.
\end{methoddesc}
\begin{methoddesc}[Matrix]{add_element}{i, j, value}
  Only on \class{DenseMatrix}, \class{SparseBuildMatrix}. Basically,
  this element is equivalent to \code{self[i,j] += value},
  however it may avoid a sort on \class{SparseBuildMatrix}
  instances and may thus run \em{significantly} faster.
\end{methoddesc}
\begin{methoddesc}[Matrix]{set_element_past_end}{i, j, value}
  Basically, this element is equivalent to \code{self[i,j] = value},
  ...
  efficient way to build \class{SparseExecuteMatrix} types
...
\begin{methoddesc}[SparseExecuteMatrix]{complete_index1_data}{}
  ...
  \class{SparseExecuteMatrix}'s storage consists of a list of 
